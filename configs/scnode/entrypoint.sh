...
# overwrite default BLOCKS FORGING RATE if provided
if [ -n "${SCNODE_FORGER_BLOCK_TIME:-}" ]; then
  SCNODE_FORGER_BLOCK_TIME="$(echo -en "\n        forgingFrequencyInSeconds = ${SCNODE_FORGER_BLOCK_TIME}")"
fi
export SCNODE_FORGER_BLOCK_TIME
...
'$SCNODE_GENESIS_WITHDRAWALEPOCHLENGTH:$SCNODE_GENESIS_COMMTREEHASH:$SCNODE_GENESIS_ISNONCEASING:$SCNODE_FORGER_ALLOWED_FORGERS:$SCNODE_FORGER_ENABLED:$SCNODE_FORGER_RESTRICT:$SCNODE_FORGER_BLOCK_TIME:'\
'$SCNODE_NET_DECLAREDADDRESS:$SCNODE_NET_KNOWNPEERS:$SCNODE_NET_MAGICBYTES:$SCNODE_NET_NODENAME:$SCNODE_NET_P2P_PORT:$MAX_INCOMING_CONNECTIONS:$MAX_OUTGOING_CONNECTIONS'\
'$SCNODE_WALLET_GENESIS_SECRETS:$SCNODE_WALLET_SEED:$SCNODE_WALLET_MAXTX_FEE:$SCNODE_WS_ZEN_IP:$SCNODE_WS_ZEN_PORT'
export SUBST
